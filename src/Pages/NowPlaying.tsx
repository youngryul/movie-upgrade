...
import {getNow, getPopular, IGetMoviesResult} from "../api";
...
function NowPlaying() {
    const { data, isLoading } = useQuery<IGetMoviesResult>(["movies", "now"], getNow);
    const popularMatch: PathMatch<string> | null = useMatch("/now/:id");
    const navigate = useNavigate();

    const onBoxClicked = (movieId: number) => {
        navigate(`/now/${movieId}`);
    };

    const clickedMovie = popularMatch?.params.id &&
        data?.results.find((movie) => movie.id === +popularMatch.params.id!);

    const onClose = () => navigate("/now-playing");
